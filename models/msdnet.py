...
class MSDLayer(nn.Module):
    ...
    def convolve(self, in_channels, out_channels, conv_type,
        ...
        if bottleneck:
            ...
            conv.add_module('Bottleneck_1x1', nn.Conv1d(in_channels,
                                                        tmp_channels,
                                                        kernel_size=1,
                                                        stride=1,
                                                        padding=0))
            conv.add_module('Bottleneck_BN', nn.BatchNorm1d(tmp_channels))
        ...
        conv.add_module('BN_out', nn.BatchNorm1d(out_channels))
...
class OpportunityClassifier(nn.Module):
    ...
    def __init__(self, num_channels, num_classes):
        ...
        super(OpportunityClassifier, self).__init__()
        self.inner_channels = 128

        self.features = nn.Sequential(
            nn.Conv1d(num_channels, self.inner_channels, kernel_size=3,
                      stride=2, padding=1),
            nn.BatchNorm1d(self.inner_channels),
            nn.ReLU(inplace=True),
            nn.Conv1d(self.inner_channels, self.inner_channels, kernel_size=3,
                      stride=2, padding=1),
            nn.BatchNorm1d(self.inner_channels),
            nn.ReLU(inplace=True),
            nn.AvgPool1d(4)
...
class MSDNet(nn.Module):
    ...
    def build_modules(self, num_channels):
        ...
        for i in range(0, self.num_blocks):
            ...
            modules[i + self.num_blocks] = \
                OpportunityClassifier(channels_in_last_layer, self.num_classes)
        return modules
    ...
    def create_block(self, num_channels, block_num):
        ...
        if block_num == 0:
            block.add_module('MSD_first', MSDFirstLayer(self.in_channels,
                                                        num_channels,
                                                        self.num_scales,
                                                        self.args))
