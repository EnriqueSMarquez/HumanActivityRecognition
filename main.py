...
model = resnets.HAR_ResNet1D(input_channels=training_data.data['inputs'].shape[1],kernel_size=kernel_size,depth=depth)
...
model = model.cuda()
optimizer = torch.optim.Adam(model.parameters(),lr=0.001,weight_decay=10.e-4,amsgrad=True)
